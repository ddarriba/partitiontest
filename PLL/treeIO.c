...
stringHashtable *initStringHashTable(hashNumberType n) {
	...
	hashNumberType tableSize, i, primeTableLength = sizeof(initTable)
			/ sizeof(initTable[0]), maxSize = (hashNumberType) -1;
	...
	h->table = (stringEntry**) rax_calloc((size_t) tableSize,
			sizeof(stringEntry*));
...
double getBranchLength(pllInstance *tr, partitionList *pr, int perGene,
		nodeptr p) {
	double z = 0.0, x = 0.0;
...
static char *TreeInner2StringREC(char *treestr, pllInstance *tr,
		partitionList *pr, nodeptr p, boolean printBranchLengths,
		boolean printNames, boolean printLikelihood, boolean rellTree,
		boolean finalPrint, int perGene, boolean branchLabelSupport,
		boolean printSHSupport, boolean printInnerNodes) {
	...
	if (isTip(p->number, tr->mxtips)) {
		...
		*treestr++ = '(';
		treestr = TreeInner2StringREC(treestr, tr, pr, p->next->back,
				printBranchLengths, printNames, printLikelihood, rellTree,
				finalPrint, perGene, branchLabelSupport, printSHSupport,
				printInnerNodes);
		*treestr++ = ',';
		treestr = TreeInner2StringREC(treestr, tr, pr, p->next->next->back,
				printBranchLengths, printNames, printLikelihood, rellTree,
				finalPrint, perGene, branchLabelSupport, printSHSupport,
				printInnerNodes);
		if (p == tr->start->back) {
			*treestr++ = ',';
			treestr = TreeInner2StringREC(treestr, tr, pr, p->back,
					printBranchLengths, printNames, printLikelihood, rellTree,
					finalPrint, perGene, branchLabelSupport, printSHSupport,
					printInnerNodes);
		}
			...
			if ((!isTip(p->number, tr->mxtips))
				...
				if (printSHSupport)
					sprintf(treestr, ":%8.20f[%d]",
							getBranchLength(tr, pr, perGene, p),
							p->bInf->support);
				if (printInnerNodes)
					sprintf(treestr, "[%d]", p->number);
...
static char *Tree2StringREC(char *treestr, pllInstance *tr, partitionList *pr,
		nodeptr p, boolean printBranchLengths, boolean printNames,
		boolean printLikelihood, boolean rellTree, boolean finalPrint,
	...
	if (isTip(p->number, tr->mxtips)) {
		...
		*treestr++ = '(';
		treestr = Tree2StringREC(treestr, tr, pr, p->next->back,
				printBranchLengths, printNames, printLikelihood, rellTree,
				finalPrint, perGene, branchLabelSupport, printSHSupport);
		*treestr++ = ',';
		treestr = Tree2StringREC(treestr, tr, pr, p->next->next->back,
				printBranchLengths, printNames, printLikelihood, rellTree,
				finalPrint, perGene, branchLabelSupport, printSHSupport);
		if (p == tr->start->back) {
			*treestr++ = ',';
			treestr = Tree2StringREC(treestr, tr, pr, p->back,
					printBranchLengths, printNames, printLikelihood, rellTree,
					finalPrint, perGene, branchLabelSupport, printSHSupport);
		}
		*treestr++ = ')';
...
void printTopology(pllInstance *tr, partitionList *pr, boolean printInner) {
	if (!printInner) {
		...
		Tree2String(tr->tree_string, tr, pr, tr->start->back,
				printBranchLengths, 0, 0, 0, 0, PLL_SUMMARIZE_LH, 0, 0);
		fprintf(stderr, "%s", tr->tree_string);
	} else {
		TreeInner2StringREC(tr->tree_string, tr, pr, tr->start->back, PLL_FALSE,
				0, 0, 0, 0, PLL_SUMMARIZE_LH, 0, 0, PLL_TRUE);
		fprintf(stderr, "%s", tr->tree_string);
		fprintf(stderr, "Start was %d, pnb %d, pnnb %d, pback %d\n",
				tr->start->back->number, tr->start->back->next->back->number,
				tr->start->back->next->next->back->number, tr->start->number);
	}
}

char *Tree2String(char *treestr, pllInstance *tr, partitionList *pr, nodeptr p,
	...
	Tree2StringREC(treestr, tr, pr, p, printBranchLengths, printNames,
			printLikelihood, rellTree, finalPrint, perGene, branchLabelSupport,
			printSHSupport);
...
void printTreePerGene(pllInstance *tr, partitionList *pr, analdef *adef,
		char *fileName, char *permission) {
	FILE *treeFile;
	...
	for (i = 0; i < numberOfModels; i++) {
		...
		Tree2String(tr->tree_string, tr, pr, tr->start->back, PLL_TRUE,
				PLL_TRUE, PLL_FALSE, PLL_FALSE, PLL_TRUE, i, PLL_FALSE,
				PLL_FALSE);
...
static void treeEchoContext(FILE *fp1, FILE *fp2, int n) { /* treeEchoContext */
	int ch;
	boolean waswhite;
...
static boolean treeProcessLength(FILE *fp, double *dptr) {
	...
	if ((ch = treeGetCh(fp)) == EOF)
		return PLL_FALSE; /*  Skip comments */
	(void) ungetc(ch, fp);
...
static boolean addElementLen(FILE *fp, pllInstance *tr, nodeptr p,
		boolean readBranchLengths, boolean readNodeLabels, int *lcount) {
	nodeptr q;
	int n, ch, fres;
	...
	if ((ch = treeGetCh(fp)) == '(') {
		...
		if (!treeNeedCh(fp, ',', "in"))
			return PLL_FALSE;
		if (!addElementLen(fp, tr, q->next->next, readBranchLengths,
				readNodeLabels, lcount))
			return PLL_FALSE;
		if (!treeNeedCh(fp, ')', "in"))
			return PLL_FALSE;
...
static nodeptr uprootTree(pllInstance *tr, nodeptr p, boolean readBranchLengths,
		boolean readConstraint, int numBranches) {
	nodeptr q, r, s, start;
	int n, i;
...
int treeReadLen(FILE *fp, pllInstance *tr, boolean readBranches,
	...
	if (!tr->rooted) {
		if ((ch = treeGetCh(fp)) == ',') {
			if (!addElementLen(fp, tr, p->next->next, readBranches,
					readNodeLabels, &lcount))
				assert(0);
	...
	if (tr->rooted) {
		...
		p->next->next->back = (nodeptr) NULL;
		//DIEGO: CHECK THIS
		tr->start = uprootTree(tr, p->next->next, PLL_FALSE, PLL_FALSE,
				NUM_BRANCHES);
		if (!tr->start) {
