 ...
 */
void regionalSmooth(pllInstance *tr, partitionList *pr, nodeptr p, int maxtimes,
		int region) {
	nodeptr q;
	int i;
	int numBranches = pr->perGeneBranchLengths ? pr->numberOfPartitions : 1;
...
boolean insertBIG(pllInstance *tr, partitionList *pr, nodeptr p, nodeptr q) {
	...
	if (tr->thoroughInsertion) {
		double zqr[numBranches], zqs[numBranches], zrs[numBranches], lzqr, lzqs,
		...
		makenewzGeneric(tr, pr, q, s, defaultArray, PLL_ITERATIONS, zqs,
				PLL_FALSE);
		makenewzGeneric(tr, pr, r, s, defaultArray, PLL_ITERATIONS, zrs,
				PLL_FALSE);
...
boolean insertRestoreBIG(pllInstance *tr, partitionList *pr, nodeptr p,
		nodeptr q) {
	nodeptr r, s;
...
void addTraverseBIG(pllInstance *tr, partitionList *pr, nodeptr p, nodeptr q,
		int mintrav, int maxtrav) {
	if (--mintrav <= 0) {
		if (!testInsertBIG(tr, pr, p, q))
			return;
...
int rearrangeBIG(pllInstance *tr, partitionList *pr, nodeptr p, int mintrav,
		int maxtrav) {
	...
	if (!isTip(p->number, tr->mxtips) && doP) {
		...
		if (!isTip(p1->number, tr->mxtips) || !isTip(p2->number, tr->mxtips)) {
			...
			if (!removeNodeBIG(tr, pr, p, numBranches))
				return PLL_BADREAR;

			if (!isTip(p1->number, tr->mxtips)) {
				addTraverseBIG(tr, pr, p, p1->next->back, mintrav, maxtrav);

				addTraverseBIG(tr, pr, p, p1->next->next->back, mintrav,
						maxtrav);
			}

			if (!isTip(p2->number, tr->mxtips)) {
				addTraverseBIG(tr, pr, p, p2->next->back, mintrav, maxtrav);
				addTraverseBIG(tr, pr, p, p2->next->next->back, mintrav,
						maxtrav);
			}
		...
		if ((!isTip(q1->number, tr->mxtips)
				...
				|| (!isTip(q2->number, tr->mxtips)
						&& (!isTip(q2->next->back->number, tr->mxtips)
								|| !isTip(q2->next->next->back->number,
										tr->mxtips)))) {
			...
			if (!removeNodeBIG(tr, pr, q, numBranches))
				return PLL_BADREAR;

			mintrav2 = mintrav > 2 ? mintrav : 2;

			if (/*! q1->tip*/!isTip(q1->number, tr->mxtips)) {
				addTraverseBIG(tr, pr, q, q1->next->back, mintrav2, maxtrav);
				addTraverseBIG(tr, pr, q, q1->next->next->back, mintrav2,
						maxtrav);
			}

			if (/*! q2->tip*/!isTip(q2->number, tr->mxtips)) {
				addTraverseBIG(tr, pr, q, q2->next->back, mintrav2, maxtrav);
				addTraverseBIG(tr, pr, q, q2->next->next->back, mintrav2,
						maxtrav);
			}
...
boolean testInsertRestoreBIG(pllInstance *tr, partitionList *pr, nodeptr p,
		nodeptr q) {
	if (tr->thoroughInsertion) {
		if (!insertBIG(tr, pr, p, q))
			return PLL_FALSE;
...

		{
			...
			if (!isTip(x->number, tr->mxtips)
					&& !isTip(y->number, tr->mxtips)) {
				while ((!x->x) || (!y->x)) {
					if (!(x->x))
						newviewGeneric(tr, pr, x, PLL_FALSE);
					if (!(y->x))
...
static void writeCheckpoint(pllInstance *tr, partitionList *pr, int state) {
	...
	for (model = 0; model < pr->numberOfPartitions; model++) {
		int dataType = pr->partitionData[model]->dataType;

		myfwrite(&(pr->partitionData[model]->numberOfCategories), sizeof(int),
				1, f);
		myfwrite(pr->partitionData[model]->perSiteRates, sizeof(double),
				tr->maxCategories, f);
		myfwrite(pr->partitionData[model]->EIGN, sizeof(double),
				pLengths[dataType].eignLength, f);
		myfwrite(pr->partitionData[model]->EV, sizeof(double),
				pLengths[dataType].evLength, f);
		myfwrite(pr->partitionData[model]->EI, sizeof(double),
				pLengths[dataType].eiLength, f);

		myfwrite(pr->partitionData[model]->frequencies, sizeof(double),
				pLengths[dataType].frequenciesLength, f);
		myfwrite(pr->partitionData[model]->tipVector, sizeof(double),
				pLengths[dataType].tipVectorLength, f);
		myfwrite(pr->partitionData[model]->substRates, sizeof(double),
		...
		if (pr->partitionData[model]->protModels == LG4) {
			...
			for (k = 0; k < 4; k++) {
				myfwrite(pr->partitionData[model]->EIGN_LG4[k], sizeof(double),
						pLengths[dataType].eignLength, f);
				myfwrite(pr->partitionData[model]->EV_LG4[k], sizeof(double),
						pLengths[dataType].evLength, f);
				myfwrite(pr->partitionData[model]->EI_LG4[k], sizeof(double),
						pLengths[dataType].eiLength, f);
				myfwrite(pr->partitionData[model]->frequencies_LG4[k],
						sizeof(double), pLengths[dataType].frequenciesLength,
						f);
				myfwrite(pr->partitionData[model]->tipVector_LG4[k],
						sizeof(double), pLengths[dataType].tipVectorLength, f);
				myfwrite(pr->partitionData[model]->substRates_LG4[k],
						sizeof(double), pLengths[dataType].substRatesLength, f);
			}
		}
...
static void readCheckpoint(pllInstance *tr, partitionList *pr) {
	int restartErrors = 0, model;

	FILE *f = myfopen(binaryCheckpointInputName, "r");
	...
	if (tr->ckp.searchConvergenceCriterion != tr->searchConvergenceCriterion) {
		printf(
				"restart error, you are trying to re-start a run where the ML search criterion was turned %s\n",
	...
	if (tr->ckp.rateHetModel != tr->rateHetModel) {
		printf(
				"restart error, you are trying to re-start a run with a different model of rate heterogeneity, the checkpoint was obtained under: %s\n",
	...
	if (tr->ckp.maxCategories != tr->maxCategories) {
		printf(
				"restart error, you are trying to re-start a run with %d per-site rate categories, the checkpoint was obtained with: %d\n",
	...
	if (tr->ckp.NumberOfModels != pr->numberOfPartitions) {
		printf(
				"restart error, you are trying to re-start a run with %d partitions, the checkpoint was obtained with: %d partitions\n",
				(int) pr->numberOfPartitions, tr->ckp.NumberOfModels);
		restartErrors++;
	}

	if (tr->ckp.numBranches != pr->perGeneBranchLengths ?
			pr->numberOfPartitions : 1) {
		printf(
				"restart error, you are trying to re-start a run where independent per-site branch length estimates were turned %s\n",
	...
	if (tr->ckp.originalCrunchedLength != tr->originalCrunchedLength) {
		printf(
				"restart error, you are trying to re-start a run with %d site patterns, the checkpoint was obtained with: %d site patterns\n",
				tr->ckp.originalCrunchedLength, tr->originalCrunchedLength);
		restartErrors++;
	}
	...
	if (tr->ckp.mxtips != tr->mxtips) {
		printf(
				"restart error, you are trying to re-start a run with %d taxa, the checkpoint was obtained with: %d taxa\n",
	...
	if (strcmp(tr->ckp.seq_file, seq_file) != 0) {
		printf(
				"restart error, you are trying to re-start from alignemnt file %s, the checkpoint was obtained with file: %s\n",
	...
	if (restartErrors > 0) {
		printf(
				"User induced errors with the restart from checkpoint, exiting ...\n");

		if (restartErrors > 4)
			printf(
					" ... maybe you should do field work instead of trying to use a computer ...\n");
		if (restartErrors > 6)
	...
	if (tr->searchConvergenceCriterion) {
		...
		if ((tr->ckp.state == FAST_SPRS && tr->ckp.fastIterations > 0)
			...
			bitVectorInitravSpecial(tr->bitVectors, tr->nodep[1]->back,
					tr->mxtips, tr->vLength, tr->h, 0, BIPARTITIONS_RF,
					(branchInfo *) NULL, &bCounter, 1, PLL_FALSE, PLL_FALSE,
					tr->threadID);
		...
		if ((tr->ckp.state == FAST_SPRS && tr->ckp.fastIterations > 1)
			...
			bitVectorInitravSpecial(tr->bitVectors, tr->nodep[1]->back,
					tr->mxtips, tr->vLength, tr->h, 1, BIPARTITIONS_RF,
					(branchInfo *) NULL, &bCounter, 1, PLL_FALSE, PLL_FALSE,
					tr->threadID);
	...
	for (model = 0; model < pr->numberOfPartitions; model++) {
		...
		myfread(&(pr->partitionData[model]->numberOfCategories), sizeof(int), 1,
				f);
		myfread(pr->partitionData[model]->perSiteRates, sizeof(double),
				tr->maxCategories, f);
		myfread(pr->partitionData[model]->EIGN, sizeof(double),
				pLengths[dataType].eignLength, f);
		myfread(pr->partitionData[model]->EV, sizeof(double),
				pLengths[dataType].evLength, f);
		myfread(pr->partitionData[model]->EI, sizeof(double),
				pLengths[dataType].eiLength, f);

		myfread(pr->partitionData[model]->frequencies, sizeof(double),
				pLengths[dataType].frequenciesLength, f);
		myfread(pr->partitionData[model]->tipVector, sizeof(double),
				pLengths[dataType].tipVectorLength, f);
		myfread(pr->partitionData[model]->substRates, sizeof(double),
				pLengths[dataType].substRatesLength, f);
		...
		if (pr->partitionData[model]->protModels == LG4) {
			...
			for (k = 0; k < 4; k++) {
				myfread(pr->partitionData[model]->EIGN_LG4[k], sizeof(double),
						pLengths[dataType].eignLength, f);
				myfread(pr->partitionData[model]->EV_LG4[k], sizeof(double),
						pLengths[dataType].evLength, f);
				myfread(pr->partitionData[model]->EI_LG4[k], sizeof(double),
						pLengths[dataType].eiLength, f);
				myfread(pr->partitionData[model]->frequencies_LG4[k],
						sizeof(double), pLengths[dataType].frequenciesLength,
						f);
				myfread(pr->partitionData[model]->tipVector_LG4[k],
						sizeof(double), pLengths[dataType].tipVectorLength, f);
				myfread(pr->partitionData[model]->substRates_LG4[k],
						sizeof(double), pLengths[dataType].substRatesLength, f);
			}
		}

		makeGammaCats(pr->partitionData[model]->alpha,
				pr->partitionData[model]->gammaRates, 4, tr->useMedian);
	}
...
int determineRearrangementSetting(pllInstance *tr, partitionList *pr,
		analdef *adef, bestlist *bestT, bestlist *bt) {
	const
	int MaxFast = 26;
	...
	boolean impr = PLL_TRUE, cutoff = tr->doCutoff;
...
void evaluate(pllInstance *tr, partitionList *pr, analdef *adef,
	...
	while (impr) {
		...
		if (tr->searchConvergenceCriterion) {
			...
			if (fastIterations > 0) {
				...
				{
					printBothOpen(
							"ML fast search converged at fast SPR cycle %d with stopping criterion\n",
							fastIterations);
					printBothOpen(
							"Relative Robinson-Foulds (RF) distance between respective best trees after one succseful SPR cycle: %f%s\n",
							rrf, "%");
					...
					printBothOpen(
							"ML search convergence criterion fast cycle %d->%d Relative Robinson-Foulds %f\n",
		...
		if (impr) {
			...
			if (tr->searchConvergenceCriterion) {
				...
				if (thoroughIterations > 0) {
					...
					{
						printBothOpen(
								"ML search converged at thorough SPR cycle %d with stopping criterion\n",
								thoroughIterations);
						printBothOpen(
								"Relative Robinson-Foulds (RF) distance between respective best trees after one succseful SPR cycle: %f%s\n",
								rrf, "%");
						...
						printBothOpen(
								"ML search convergence criterion thorough cycle %d->%d Relative Robinson-Foulds %f\n",
								thoroughIterations - 1, thoroughIterations,
								rrf);
				}
			}
...
void computeBIGRAPID(pllInstance *tr, partitionList *pr, analdef *adef,
		boolean estimateModel) {
	int i, impr, bestTrav = 0, rearrangementsMax = 0, rearrangementsMin = 0,
			thoroughIterations = 0, fastIterations = 0;
	...
	while (1) {
		/* once again if we want to restart from a checkpoint that was written during this loop we need
		 to restore the values of the variables appropriately */
		START_SLOW_SPRS: if (adef->useCheckpoint && tr->ckp.state == SLOW_SPRS) {
			impr = tr->ckp.impr;
			bestTrav = tr->ckp.bestTrav;
			rearrangementsMax = tr->ckp.rearrangementsMax;
			rearrangementsMin = tr->ckp.rearrangementsMin;
...
boolean treeEvaluate(pllInstance *tr, partitionList *pr,
		int maxSmoothIterations) /* Evaluate a user tree */
{
