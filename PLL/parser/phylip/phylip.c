...
printTokens (int input)
{
  ...
  while (token.tclass != LEX_EOF && token.tclass != LEX_UNKNOWN);

  if (token.tclass == LEX_UNKNOWN) return (0);
...
read_phylip_header (char * rawdata, int * inp, int * nTaxa, int * seqLen)
{
  ...
  if (token.tclass != LEX_NUMBER) return (0);
    ...
    if (i < phylip->nTaxa) 
     {
       ...
       if (token.tclass != LEX_STRING && token.tclass != LEX_NUMBER && token.tclass != LEX_FLOAT)
        {
          free (seqLen);
          return (0);
        }
    ...
    while (1)
     {
       ...
       if (token.tclass == LEX_NEWLINE) break;

       if (token.tclass != LEX_STRING)
        {
          free (seqLen);
          return (0);
        }
