...
namespace partest
{
                                   ...
                                   InformationCriterion _ic, double _sampleSize)
  {
    ...
    for (size_t i = 0; i < modelset.size (); i++)
    {
      ...
      double value = computeIc (_ic, model->getLnL (), freeParameters,
                                _sampleSize);
      double bicScore = computeIc (BIC, model->getLnL (), freeParameters,
                                   _sampleSize);
      double aicScore = computeIc (AIC, model->getLnL (), freeParameters,
                                   _sampleSize);
      double aiccScore = computeIc (AICC, model->getLnL (), freeParameters,
                                    _sampleSize);
  ...
  void ModelSelector::print (ostream& out)
  {
    ...
    out << setw (5) << "###" << setw (15) << "Model" << setw (10) << "K"
        ...
        << setw (15) << setprecision (4) << "Weight" << setw (15) << "CumWeight"
        << endl;
    out << setw (115) << setfill ('-') << "" << setfill (' ') << endl;
    for (size_t i = 0; i < selectionModels->size (); i++)
    {
    ...
    for (size_t i = 0; i < selectionModels->size (); i++)
    {
      ...
      out << setw (5) << i + 1 << setw (20) << fixed << setprecision (4)
          << selectionModel->getBicScore () << setw (20) << fixed
          << setprecision (4) << selectionModel->getAicScore () << setw (20)
          << fixed << setprecision (4) << selectionModel->getAiccScore ()
          << endl;
    }
    ...
    for (size_t i = 0; i < selectionModels->size (); i++)
    {
      ...
      if (data_type == DT_NUCLEIC)
      {
        ...
        {
          out << setw (10) << setprecision (4) << model->getFrequencies ()[j];
        }
