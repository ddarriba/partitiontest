...
namespace partest
{
  ...
  PartitionSelector::PartitionSelector (
      vector<PartitioningScheme *> _schemesArray) :
...
// #pragma omp parallel for schedule(dynamic)
    for (size_t part_id = 0; part_id < schemesArray.size (); part_id++)
    {
      ...
      (schemesVector->at (part_id))->numParameters =
          scheme->getNumberOfFreeParameters ();
  ...
  void PartitionSelector::print (ostream& out, int limit)
  {
    ...
    for (size_t id = 0; id < maxSchemes; id++)
    {
      ...
      out << setw (7) << id + 1 << setw (5) << " "
          << sp->scheme->getCode (numCodeLines - 1) << endl;
    ...
    out << setw (barLength) << setfill ('-') << "" << setfill (' ') << endl;
    out << setw (7) << "###" << setw (10) << "N" << setw (10) << "K"
        << setw (20) << "lnL     " << setw (20) << "Value     " << endl;
    out << setw (barLength) << setfill ('-') << "" << setfill (' ') << endl;
    for (size_t id = 0; id < maxSchemes; id++)
    {
      SelectionPartitioningScheme * sp = schemesVector->at (id);
  ...
  void SelectionPartitioningScheme::print (ostream & out)
  {
    ...
    out << setw (5) << "###" << setw (15) << "Model" << setw (5) << "K"
        << setw (15) << "lnL" << setw (15) << "Value" << setw (15) << "Delta"
        << setw (15) << "Weight" << setw (15) << "CumWeight" << endl;
    out << setw (100) << setfill ('-') << "" << setfill (' ') << endl;
