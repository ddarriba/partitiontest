...
namespace partest
{
  ...
  {
    if (n <= 0)
      throw std::invalid_argument ("Stirling::iterator: n must be positive");
      ...
      iterator (n, first), psize (psize)
  {
    if (psize <= 0 || psize > n)
      throw std::invalid_argument (
          "Stirling::iterator_k: psize must be in [1..n]");
    ...
    {
      it_size += pow (-1, psize - i) * Utilities::combinatorial (psize, i)
          * pow (i, n);
    }
  ...
  Stirling::iterator_k * Stirling::get_partitions (
      int k, t_schemesVector* &schemeVector, t_partitioningScheme* &elements)
  {
