...
void PrintMeta::print_options(ostream& output, ParTestOptions & options) {
  ...
  if (options.getInputFile().length()
  ...
  output << setw(OPT_DESCR_LENGTH - 5) << left << "     Number of taxa:"
      << setw(10) << right << options.getAlignment()->getNumSeqs() << endl;
  output << setw(OPT_DESCR_LENGTH - 5) << left
      << "     Number of sites (total):"
      << setw(10) << right << options.getAlignment()->getNumSites() << endl;
  output << setw(OPT_DESCR_LENGTH - 5) << left
      << "     Number of unique patterns:"
      << setw(10) << right << options.getAlignment()->getNumPatterns() << endl;

  if (options.getTreeFile()) {
    output << setw(OPT_DESCR_LENGTH) << left << "  Input tree:";
    if (strlen(options.getTreeFile())
        > (H_RULE_LENGTH - OPT_DESCR_LENGTH)) {
      output << endl << setw(H_RULE_LENGTH - strlen(options.getTreeFile()))
          << " ";
    }
  ...
  if (strlen(options.getConfigFile())
      > (H_RULE_LENGTH - OPT_DESCR_LENGTH)) {
...
void PrintMeta::print_usage(std::ostream& out) {
  ...
  out << setw(SHORT_OPT_LENGTH) << " " << setw(COMPL_OPT_LENGTH)
      << "--data-type nt"
      << "Nucleotide sequences (DEFAULT)" << endl;
  out << setw(SHORT_OPT_LENGTH) << " " << setw(COMPL_OPT_LENGTH)
      << "--data-type aa"
      << "Amino-acid sequences" << endl;
  ...
  out << setw(MAX_OPT_LENGTH) << " "
      << "(DEFAULT) Slowest but also more accurate"
      << endl;
  out << setw(SHORT_OPT_LENGTH) << " " << setw(COMPL_OPT_LENGTH)
      << "--topology bionj"
  ...
  out << setw(MAX_OPT_LENGTH) << left << "  -u, --user-tree TREE_FILE"
      ...
      << endl;
  out << setw(MAX_OPT_LENGTH) << " "
      << "with a starting topology different than \"user-defined\"" << endl;
  out << setw(MAX_OPT_LENGTH) << " " << "The tree must be in Newick format"
      << endl;
  out << setw(MAX_OPT_LENGTH) << left << "  -I, --invariant-sites"
      << "Includes models with a proportion of invariant sites (+I)" << endl;
  ...
  out << setw(SHORT_OPT_LENGTH) << " " << setw(COMPL_OPT_LENGTH)
      ...
      << endl;
  out << setw(SHORT_OPT_LENGTH) << " " << setw(COMPL_OPT_LENGTH)
      << "--selection-criterion aic" << "Akaike Information Criterion" << endl;
  out << setw(SHORT_OPT_LENGTH) << " " << setw(COMPL_OPT_LENGTH)
  ...
  out << setw(MAX_OPT_LENGTH) << " "
      << "Configuration file defines the gene bounds in the alignment" << endl;
  out << setw(MAX_OPT_LENGTH) << " " << "Format should be as follows:" << endl;
  out << setw(MAX_OPT_LENGTH) << " "
