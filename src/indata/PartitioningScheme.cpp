...
namespace partest
{
      ...
      for (size_t i = 1; i < numberOfElements; i++)
      {
        for (size_t j = 0; j < i; j++)
        {
          ...
          if (starting_topology == StartTopoFIXED)
          {
            ...
            for (int cBranch = 0;
                cBranch < Utilities::numberOfBranches ((int) num_taxa);
                cBranch++)
            {
  ...
  unsigned int PartitioningScheme::getNumberOfFreeParameters ()
  {
    if (!isOptimized ())
      return 0;
    unsigned int k =
        reoptimize_branch_lengths ?
            0 : (unsigned int) Utilities::numberOfBranches ((int) num_taxa);
    for (size_t i = 0; i < partitions.size (); i++)
    {
      PartitionElement * pe = partitions.at (i);
      ...
      {
        k +=
            (unsigned int) pe->getBestModel ()->getModel ()->getNumberOfFreeParameters ();
      }
      else
      {
        k +=
            (unsigned int) pe->getBestModel ()->getModel ()->getModelFreeParameters ();
      }
    }
    return k;
  }
  ...
  double PartitioningScheme::getLinkedIcValue ()
  {
    if (!isOptimized ())
      return 0.0;
    return ModelSelector::computeIc (ic_type, getLnL (),
  ...
  double PartitioningScheme::getLinkedBicValue ()
  {
    if (!isOptimized ())
      return 0.0;
    return ModelSelector::computeIc (BIC, getLnL (),
                                     (int) getNumberOfFreeParameters (),
                                     seq_len);
  }
  ...
  double PartitioningScheme::getLinkedAicValue ()
  {
    if (!isOptimized ())
      return 0.0;
    return ModelSelector::computeIc (AIC, getLnL (),
                                     (int) getNumberOfFreeParameters (),
                                     seq_len);
  }
  ...
  double PartitioningScheme::getLinkedAiccValue ()
  {
    if (!isOptimized ())
      return 0.0;
    return ModelSelector::computeIc (AICC, getLnL (),
                                     (int) getNumberOfFreeParameters (),
                                     seq_len);
  }
  ...
  double PartitioningScheme::getLinkedDTValue ()
  {
    if (!isOptimized ())
      return 0.0;
    return ModelSelector::computeIc (DT, getLnL (),
                                     (int) getNumberOfFreeParameters (),
                                     seq_len);
  }
  ...
  void PartitioningScheme::print (ostream & out)
  {
    ...
    if (isOptimized ())
    {
      out << setw (23) << "LnL:" << fixed << setprecision (4) << getLnL ()
          << endl;
      out << setw (23) << "Num.Parameters:" << getNumberOfFreeParameters ()
          << endl;
      out << setw (23) << "Selection score:" << fixed << setprecision (4)
          << getIcValue () << endl;
      out << setw (23) << "BIC score:" << fixed << setprecision (4)
          << getBicValue () << endl;
      out << setw (23) << "AIC score:" << fixed << setprecision (4)
          << getAicValue () << endl;
      out << setw (23) << "AICC score:" << fixed << setprecision (4)
          << getAiccValue () << endl;
      out << setw (23) << "Linked score:" << fixed << setprecision (4)
          << getLinkedIcValue () << endl;
      out << setw (23) << "Linked BIC_score:" << fixed << setprecision (4)
          << getLinkedBicValue () << endl;
      out << setw (23) << "Linked AIC_score:" << fixed << setprecision (4)
          << getLinkedAicValue () << endl;
      out << setw (23) << "Linked AICC_score:" << fixed << setprecision (4)
          << getLinkedAiccValue () << endl;
      if (tree > 0)
      {
        out << "Tree:         " << getTree () << endl;
      }
      out << endl;
      out << setw (6) << "--ID--" << setw (11) << " --Model-- " << setw (5)
          << " --K-" << setw (15) << " -----LnL------" << setw (15)
          << " -----BIC------" << setw (15) << " -----AIC------" << setw (15)
          << " -----AICc-----" << endl;
      for (size_t i = 0; i < numberOfElements; i++)
      {
        out << setw (6) << left << i + 1 << setw (11) << left
            << getElement (i)->getBestModel ()->getModel ()->getName ()
            << setw (5) << right
            << getElement (i)->getBestModel ()->getModel ()->getNumberOfFreeParameters ()
            << setw (15) << right << fixed << setprecision (4)
            << getElement (i)->getBestModel ()->getModel ()->getLnL ()
            << setw (15) << right << fixed << setprecision (4)
            << getElement (i)->getBestModel ()->getBicScore () << setw (15)
            << right << fixed << setprecision (4)
            << getElement (i)->getBestModel ()->getAicScore () << setw (15)
            << right << fixed << setprecision (4)
            << getElement (i)->getBestModel ()->getAiccScore () << endl;
      }
    }
  }
