...
namespace partest_parser
{
  ...
  int PartestParserUtils::parsePartitionFinderFile (
      vector<string> ** partitions, char ** alignment, char ** models,
      pfSearch * searchAlgo, selection * icSelection)
  {
    ...
    for (size_t partitionId = 0; partitionId < num_genes; partitionId++)
    {
      ...
      replace (keys->at (partitionId * 2 + 1).begin (),
               keys->at (partitionId * 2 + 1).end (), '\\', '/');
      replace (keys->at (partitionId * 2 + 1).begin (),
               keys->at (partitionId * 2 + 1).end (), ';', '\0');
      partitionLine.append (" = ");
      partitionLine.append (keys->at (partitionId * 2 + 1).begin (),
                            keys->at (partitionId * 2 + 1).end ());
   ...
   */
  void PartestParserUtils::analyzeIdentifier (char **ch, char **modelName)
  {
